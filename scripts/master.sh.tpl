...
# then performs cluster bootstrap (kubeadm init + Calico + push
# join command to SSM).
#
# Injected variables:
...
#   - nlb_dns        : NLB DNS for API server endpoint
...
exec > >(tee /var/log/k8s-master.log) 2>&1
...
info "Starting K8s Control Plane bootstrap on $${NODE_HOSTNAME}"

# ════════════════════════════════════════════════════════════════
# PHASE 1: Base K8s install (identical to install-k8s.sh.tpl)
# ════════════════════════════════════════════════════════════════
...
# ─────────────────────────────────────────
# Install AWS CLI v2
# Needed for SSM Parameter Store interactions
# ─────────────────────────────────────────
...
# ════════════════════════════════════════════════════════════════
# PHASE 2: Cluster bootstrap (control plane only)
# ════════════════════════════════════════════════════════════════
...
# ─────────────────────────────────────────
# Initialize cluster
# Use local IP as endpoint first — NLB can't health-check
# us until API server is up (chicken-and-egg problem).
# We'll switch the kubeconfig endpoint to NLB later.
# ─────────────────────────────────────────
step "kubeadm init"
kubeadm init \
  --control-plane-endpoint="$${LOCAL_IP}:6443" \
  --apiserver-cert-extra-sans="$${NLB_DNS},$${LOCAL_IP}" \
...
# ─────────────────────────────────────────
step "Install Calico CNI"
retry 3 10 kubectl apply -f https://raw.githubusercontent.com/projectcalico/calico/v3.27.0/manifests/calico.yaml

info "Waiting for Calico pods to become ready..."
kubectl wait --for=condition=ready pod \
  -l k8s-app=calico-node \
  -n kube-system \
  --timeout=180s || info "Calico still starting, continuing anyway"
